...
Authorization  = מה מותר לך?</pre><ul style="margin:0;padding-right:1.2rem"><li>session</li><li>password handling</li><li>IDOR / object-level authorization</li></ul>` },
  ...
  },
  {
    ...
    subtitle: "תשובות מבוססות מידע מותר",
    phase: "ai",
    time: "45 דק'",
    capstone: "TaskFlow: «שאל על מדיניות צוות» — RAG על docs/.",
    intro: "RAG = retrieve docs + generate. Grounding = cite sources. No hallucination policy.",
    concepts: [
      { term: "RAG", def: "retrieve + augment + generate.", example: "docs/ → answer", pitfall: "no retrieval." },
      { term: "Grounding", def: "answer from sources.", example: "cite doc §3", pitfall: "fabricate." },
      { term: "Chunk", def: "doc pieces for search.", example: "500 char chunks", pitfall: "whole doc." },
      { term: "Citation", def: "link to source.", example: "[policy.md#L12]", pitfall: "no cite." },
    ],
    analogy: { text: "RAG מפריד בין מה שהמודל «יודע» מה-training לבין מה שמותר לו להגיד בהקשר זה. בלי retrieval, המודל עונה מה-training data — שעלולה להיות ישנה, שגויה, או לא רלוונטית לארגון. עם retrieval + citation — ניתן לבדוק כל תשובה.", bridge: "רשמו 3 מסמכים שיכנסו ל-docs/ של TaskFlow RAG, וכתבו שאלה אחת שצריכה להיות ל-«לא נמצא» — בדקו שהמערכת שלכם עונה נכון." },
    diagram: `<div class="flow-diagram"><div class="flow-node">Query</div><span class="flow-arrow">→</span><div class="flow-node secondary">Retrieve</div><span class="flow-arrow">→</span><div class="flow-node">Context</div><span class="flow-arrow">→</span><div class="flow-node primary">Answer + Cite</div></div>`,
    bob: { modes: ["Plan", "Agent"], workflow: "docs/ first, then RAG plan.", prompt: "Plan RAG for docs/: chunk strategy, cite format, «I don't know» rule.", promptWhy: "Trust architecture." },
    mistake: { bad: "answer without source.", good: "no cite → «לא נמצא במסמכים»." },
    lab: ["Create docs/sample.", "Chunk plan.", "Retrieve test.", "Cite check.", "Hallucination test.", "הצלחה = שאלה שיש לה תשובה ב-docs מחזירה citation; שאלה ללא מענה מחזירה «לא נמצא במסמכים»."],
    exercise: "Question with no doc — expected response?",
    quiz: [
      { q: "RAG vs fine-tune?", a: "RAG = dynamic docs; fine-tune = model change." },
      { q: "Grounding?", a: "answer tied to retrieved text." },
      { q: "No cite?", a: "say unknown — don't guess." },
    ],
    summary: ["Retrieve first.", "Always cite.", "Unknown OK.", "Test hallucination."],
  },
  {
    num: 22,
